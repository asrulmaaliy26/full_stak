...
## 🐘 6. Panduan Khusus Deploy Proyek Laravel (Backend)

Berbeda dengan proyek React (Frontend) yang hanya meng-upload folder statis (`dist/`), mendeploy proyek Laravel memerlukan sedikit modifikasi karena melibatkan proses instalasi *library* backend (Composer), *build asset* frontend (Vite), dan memastikan struktur file/permissions yang benar.

Pada proyek ini, alur deploy backend diarahkan ke subdomain **admin.staialmannan.ac.id**. Web server nantinya diarahkan ke folder `/home/staialmannan/admin.staialmannan.ac.id/backend/public`.

Kami telah menambahkan file `.github/workflows/deploy-backend.yml` untuk menangani ini. Berikut adalah perbedaan utamanya dibandingkan deploy React:

1. **Menggunakan PHP & Composer:** Runner GitHub akan menginstal PHP versi 8.2 dan menjalankan `composer install --optimize-autoloader --no-dev`.
2. **Build Vite:** Menjalankan `npm install` dan `npm run build` di dalam folder `backend/` untuk mengompilasi aset CSS/JS Laravel.
3. **Metode Archive (Tar):** Karena folder `vendor/` di Laravel memiliki ribuan file, mengirim file satu per satu menggunakan SCP akan memakan waktu sangat lama dan berisiko timeout. Sebagai gantinya, semua file dikompresi menjadi `backend.tar.gz`, dikirim ke server, lalu diekstrak otomatis melalui SSH.
4. **Permission Spesifik Laravel:** File konfigurasi `deploy-backend.yml` otomatis menjalankan `chmod -R 775` pada folder `storage` dan `bootstrap/cache` agar Laravel dapat menulis log dan cache tanpa error `Permission Denied`.
5. **Keamanan .env:** File `.env` tidak akan tertimpa otomatis jika sudah ada di server. Namun jika baru pertama kali di-deploy, sistem akan menyalin dari `.env.example`. Anda harus tetap mengatur koneksi database dan menjalankan `php artisan key:generate` secara manual di server untuk instalasi pertama.
